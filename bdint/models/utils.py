...
def ohe(train_df: pd.DataFrame, test_df: pd.DataFrame):
    # so what this should do is first fit the OneHotEncoder on the trainset using handle_unknown = 'ignore'.
    # Then we can transform the train and test set. The numerical features should not be transformed.
    # Problem is, that the result is a nparray and not pd.DataFrame. I don't no how to get it back to a pandas df
    ...
    return pd.DataFrame(train_ohe), pd.DataFrame(test_ohe)
